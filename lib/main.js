...
function gsyr( order, uplo, N, alpha, x, strideX, A, LDA ) {
	...
	if ( !isLayout( order ) ) {
		throw new TypeError( format( '2SrFx', order ) );
	}
	u = resolveStr( uplo );
	if ( u === null ) {
		throw new TypeError( format( '2SrGA', uplo ) );
	}
	if ( N < 0 ) {
		throw new RangeError( format( '2SrFz', N ) );
	}
	if ( strideX === 0 ) {
		throw new RangeError( format( '2SrGB', strideX ) );
	}
	if ( LDA < max( 1, N ) ) {
		throw new RangeError( format( '2SrGM', N, LDA ) );
	}
