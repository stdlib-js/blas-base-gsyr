...
function gsyr( uplo, N, alpha, x, strideX, offsetX, A, strideA1, strideA2, offsetA ) { // eslint-disable-line max-len
	...
	if ( u === null ) {
		throw new TypeError( format( '2SrGN', uplo ) );
	}
	if ( N < 0 ) {
		throw new RangeError( format( '2SrGE', N ) );
	}
	if ( strideX === 0 ) {
		throw new RangeError( format( '2SrGF', strideX ) );
	}
	if ( strideA1 === 0 ) {
		throw new RangeError( format( '2SrG0', strideA1 ) );
	}
	if ( strideA2 === 0 ) {
		throw new RangeError( format( '2SrGS', strideA2 ) );
	}
